    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 11, color: 'var(--t3)', marginTop: 2 }}>
            Limitler MongoDB&apos;de saklanır; harcama son 30 güne göre hesaplanır
          </p>
        </div>
      ...
      {violations > 0 && (
        <div style={{ fontSize: 11, color: 'var(--amber)', marginBottom: 10 }}>
          {violations} kategoride limit aşımı
        </div>
      )}
      ...
      {Object.keys(usage).length === 0 ? (
        <Card>
          <SectionLabel>Henüz limit yok</SectionLabel>
          <p style={{ fontSize: 12, color: 'var(--t3)', marginTop: 8 }}>
            Üstten &quot;Limit Ekle&quot; ile kategori başına aylık tavan belirleyin.
          </p>
        </Card>
      ) : (
          ...
          {Object.entries(usage).map(([cat, u]) => {
            ...
            return (
              <Card key={cat}>
                ...
                <ProgressBar pct={Math.min(u.pct, 150)} height={10} color={statusColor} />

    ...
    <div>
      <svg width="100%" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet">
        ...
        <g transform={`translate(${pad.l},${pad.t})`}>
          ...
          <polygon points={`${predPts} ${sx(data.length - 1)},${H} 0,${H}`} fill="url(#predGrad)" stroke="none" />
          <polyline points={predPts} fill="none" stroke="#00d68f" strokeWidth={1.5} strokeDasharray="4 3" />
          {actPts && (
            <>
              <polygon points={`${actPts} ${sx(actData.length - 1)},${H} 0,${H}`} fill="url(#actGrad)" stroke="none" />
